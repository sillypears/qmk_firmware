...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_BASE] = LAYOUT(
    KC_GESC,  KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_MINS, KC_EQL,  MO(_MEDIA),  KC_BSPC,          KC_PSCR, \
    ...
    KC_LCTL, KC_LGUI,  KC_LALT,         KC_SPC, KC_BSPC,                        KC_SPC,           KC_RALT, KC_RCTL,          KC_LEFT, KC_DOWN, KC_RGHT           \
  ),
...
void matrix_init_user(void) {
  ...
  setPinOutput(F5);
  writePinHigh(F5);
  // set NumLock LED to output and off (active low)asdfasdasasd~~~```~~~````~~~
  setPinOutput(F6);
  writePinHigh(F6);
